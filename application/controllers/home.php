...
class home extends CI_Controller {
	...
	private function mt_login()
	{
	$email = $this->input->post('email');
	$password = $this->input->post('password');	

	$user = $this->db->get_where('biodata', ['email' => $email])->row_array();
	// usernya ada
	if($user) {
		//jika usernya aktif
		if($user['is_active'] == 1){
	...
	public function registration()
	{
		...
		$data['regis'] = $this->db->get('master_periode')->row_array();
		
		$this->form_validation->set_rules('nisn','Nisn','trim|required|numeric|min_length[10]|max_length[10]|is_unique[biodata.nisn]',[
		'is_unique' => 'This email has already registered!',
		'numeric' => 'This nisn numerik only!',
		'min_length' => 'This nisn minimal 10 character!',
		'max_length' => 'This nisn maximal 10 character!']); 
		//form validasi
		$this->form_validation->set_rules('email','Email','required|trim|valid_email|is_unique[biodata.email]',[
		//jika email sama
		'is_unique' => 'This email has already registered!']);
		...
		$this->form_validation->set_rules('hp','Handphone','required|trim');
		$this->form_validation->set_rules('wa','Whatsapp','required|trim');
		$this->form_validation->set_rules('jk','gender','required|trim');
		$this->form_validation->set_rules('propinsi','School','required|trim');
		$this->form_validation->set_rules('jurusan','Jurusan','required|trim');
		$this->form_validation->set_rules('kota','Id Sekolah','required|trim');
		$this->form_validation->set_rules('prodi','prodi','required|trim');
		$this->form_validation->set_rules('alamat','Alamat','required|trim');
		
		
		//jika data tidak berhasil ditambahkan
		if ($this->form_validation->run() == false){
			$data['title'] = 'Beasiswa Registrasion';	
			$this->load->view('template/auth_header', $data);
			...
			$data = [
					...
					'id_sekolah' => htmlspecialchars($this->input->post('kota', true)),
					'prodi' => htmlspecialchars($this->input->post('prodi', true)),
					'alamat' => htmlspecialchars($this->input->post('alamat', true)),
					'nama' => htmlspecialchars($this->input->post('nama', true)),
					'tahapan' => htmlspecialchars($this->input->post('tahapan', true)),
					...
					//var_dump($data);
					//die;
			//siapkan token untuk dikirm ke email		
	...
	private function _sendemail($token, $type)
	{
		...
		$this->email->from('kinkay17@gmail.com', 'LP3I College Karawang');
	...
	public function forgotpassword()
	{
		...
		$this->load->view('template/auth_header', $data);
		$this->load->view('login/v_forgotpassword');
		$this->load->view('template/auth_footer');	
			
		}else{
			$email = $this->input->post('email');
			$user = $this->db->get_where('biodata',['email' => $email,'is_active' => 1])->row_array();
			if($user)
			{
				$token = base64_encode(random_bytes(32));
				$user_token = [
					...
					$this->session->set_flashdata('message', '<div class="alert alert-success" 
					role="alert">Please your email to reset your password!</div>');
	...
	public function changePassword(){
		...
		if($this->form_validation->run() == false) {
			...
			$this->load->view('template/auth_header', $data);
			$this->load->view('login/v_changepassword');
			$this->load->view('template/auth_footer');
		}else{
			$password = password_hash($this->input->post('password1'), PASSWORD_DEFAULT);
	...
	{		
		$this->load->view('login/v_blocked');
	}
	...
	public function get_kota()
	{
	   $propinsi = $_GET['propinsi'];
		  ...
		  $arr_data = array(
			 'kategori_sekolah' => $propinsi
		  );
		  $this->load->model('home_model');
		  $get_kabkota = $this->home_model->get_kota($propinsi)->result();
 
		  $arr_kabkota = array();
		  foreach($get_kabkota as $row)
		  {
			 $arr_kabkota[$row->id_sekolah] = $row->nama_sekolah;
		  }
